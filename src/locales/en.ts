// Per-language string table for en.
// One file per language so rollup can lazy-load on demand.
// Shape and fallback resolution rules live in ../locale.ts.
import type { LocaleEntry } from '../locale-types.js';

const en: LocaleEntry = {
  'tempHi': 'Temperature',
  'tempLo': 'Temperature night',
  'precip': 'Precipitations',
  'sunshine': 'Sunshine',
  'feelsLike': 'Feels like',
  'pressure_trend_rising_fast': 'Pressure rising rapidly',
  'pressure_trend_rising': 'Pressure rising',
  'pressure_trend_stable': 'Pressure stable',
  'pressure_trend_falling': 'Pressure falling',
  'pressure_trend_falling_fast': 'Pressure falling rapidly',
  'pressure_influence_rising_fast': 'rapid clearing',
  'pressure_influence_rising': 'improving',
  'pressure_influence_stable': 'no change',
  'pressure_influence_falling': 'deteriorating',
  'pressure_influence_falling_fast': 'storm likely',
  'pressure_tooltip_template': '{trend} · Δ3h {delta} hPa · {influence}',
  'dew_point_band_raureif': 'frost risk',
  'dew_point_band_nebel': 'fog risk',
  'dew_point_band_schwuel': 'muggy',
  'dew_point_band_tau': 'dew likely',
  'dew_point_band_komfort': 'comfortable',
  'dew_point_tooltip_template': 'Dew point {td} {unit} · Spread {spread} {unit} · {band}',
  'sun_strength_band_low': 'Low',
  'sun_strength_band_moderate': 'Moderate',
  'sun_strength_band_high': 'High',
  'sun_strength_band_very_high': 'Very high',
  'sun_strength_band_extreme': 'Extreme',
  'sun_strength_clear': 'clear sky',
  'sun_strength_partly': 'partly cloudy',
  'sun_strength_cloudy': 'cloudy',
  'sun_strength_protection_advised': 'sun protection advised',
  'sun_strength_night_reason': 'Sun below horizon — UV hidden',
  'units': {
    'km/h': 'km/h',
    'm/s': 'm/s',
    'mph': 'mph',
    'Bft': 'Bft',
    'hPa': 'hPa',
    'mmHg': 'mm Hg',
    'mm': 'mm',
    'in': 'in'
  },
  'cardinalDirections': [
    'N', 'N-NE', 'NE', 'E-NE', 'E', 'E-SE', 'SE', 'S-SE',
    'S', 'S-SW', 'SW', 'W-SW', 'W', 'W-NW', 'NW', 'N-NW', 'N'
  ],
  'clear-night': 'Clear, night',
  'cloudy': 'Cloudy',
  'fog': 'Fog',
  'hail': 'Hail',
  'lightning': 'Lightning',
  'lightning-rainy': 'Lightning, rainy',
  'partlycloudy': 'Partly cloudy',
  'pouring': 'Pouring',
  'rainy': 'Rainy',
  'snowy': 'Snowy',
  'snowy-rainy': 'Snowy, rainy',
  'sunny': 'Sunny',
  'windy': 'Windy',
  'windy-variant': 'Windy',
  // Canonical English strings for the visual editor. Other languages
  // should add an `editor: {…}` block matching these keys; missing
  // keys (or whole missing editor blocks) fall back to English at
  // runtime via the `t()` helper in the editor module.
  'editor': {
    'temperature': 'Temperature',
    'humidity': 'Humidity',
    'illuminance': 'Illuminance',
    'precipitation': 'Precipitation',
    'precipitation_rate': 'Precipitation rate',
    'pressure': 'Pressure',
    'wind_speed': 'Wind speed',
    'gust_speed': 'Gust speed',
    'wind_direction': 'Wind direction',
    'uv_index': 'UV index',
    'dew_point': 'Dew point',
    'sunshine_duration': 'Sunshine duration',
    'zero_degree_level': 'Zero-degree level',
    'required_marker': 'required',
    'title': 'Title',
    'days': 'Days (station)',
    'forecast_days': 'Days (forecast)',
    'weather_entity': 'Weather entity',
    // Section headings (A–F)
    'setup_heading': 'Setup',
    'sensors_heading': 'Sensors',
    'layout_heading': 'Layout',
    'style_heading': 'Style & Colours',
    'units_heading': 'Units',
    'advanced_heading': 'Advanced',
    // Section headings — top-level
    'card_setup_heading': 'Card setup',
    'station_sensors_heading': 'Your weather station\'s sensors',
    'weather_forecast_heading': 'Weather forecast',
    'chart_section_heading': 'Chart',
    'live_panel_heading': 'Live panel',
    'actions_section_heading': 'Actions',
    // Per-section reset-to-defaults button
    'reset_section': 'Reset this section to defaults',
    // Sub-section headings
    'chart_time_range_heading': 'Time range & resolution',
    // Keys retained for backwards compatibility.
    'mode_question_heading': 'What this card shows',
    'display_question_heading': 'What gets shown',
    'appearance_heading': 'Appearance',
    'tap_behavior_heading': 'Tap behavior',
    // Legacy keys (kept so older translations keep working).
    'card_heading': 'Card',
    'display_heading': 'Display',
    'chart_heading': 'Chart',
    // Mode radio
    'mode_label': 'Mode',
    'mode_station': 'Station only',
    'mode_forecast': 'Forecast only',
    'mode_combination': 'Station and Forecast Combined',
    // Chart type radio
    'chart_type_label': 'Chart type',
    'forecast_type_daily': 'Daily',
    'forecast_type_today': 'Today (24h)',
    'forecast_type_hourly': 'Hourly',
    'open_documentation': 'Open documentation',
    // Layout sub-headings
    'main_panel_heading': 'Main panel',
    'attributes_heading': 'Attributes row',
    'chart_rows_heading': 'Chart rows',
    'tap_action_label': 'Tap action',
    'hold_action_label': 'Hold action',
    'double_tap_action_label': 'Double tap action',
    'chart_appearance_heading': 'Appearance',
    // Toggles
    'show_main': 'Show "now" main panel',
    'show_temperature': 'Show current temperature',
    'show_current_condition': 'Show current condition',
    'show_attributes': 'Show attributes row',
    'show_humidity': 'Humidity',
    'show_pressure': 'Pressure',
    'show_dew_point': 'Dew point',
    'show_wind_direction': 'Wind direction',
    'show_wind_speed': 'Wind speed',
    'show_wind_gust_speed': 'Gust speed',
    'show_uv_index': 'UV index',
    'show_illuminance': 'Illuminance',
    'show_precipitation': 'Precipitation',
    'show_sun': 'Sun (sunrise / sunset)',
    'show_moon': 'Moon (illumination / rise / set)',
    'show_zero_degree_level': 'Zero-degree level',
    'layout_board_label': 'Arrangement',
    'layout_board_hint': 'Drag rows between the columns to arrange them; arrow keys work too. Saved as attributes_layout.',
    'layout_board_new_column': 'New column',
    'layout_board_reset': 'Back to the automatic arrangement',
    'show_time': 'Show clock',
    'show_time_seconds': 'Seconds',
    'show_day': 'Day name',
    'show_date': 'Date',
    'use_12hour_format': '12-hour clock',
    'show_chart_icons': 'Condition icons row',
    'show_chart_wind_direction': 'Wind direction',
    'show_chart_wind_speed': 'Wind speed',
    'show_chart_date': 'Date in chart x-axis',
    'show_chart_sunshine': 'Sunshine Bar',
    'show_chart_sunshine_hint': 'Fetches sunshine duration directly from Open-Meteo (using Home Assistant\'s configured location). No additional sensors required. Daily mode shows "Xh" labels per column; hourly mode shows bars only (full bar = full hour of sun).',
    'show_chart_mode_toggle': 'View-switch button',
    'sunshine_availability': 'Sunshine data available: {past} past days, {future} forecast days (Open-Meteo).',
    'sunshine_availability_pending': 'Sunshine: data still loading — save and reopen the editor.',
    'sunshine_availability_warning': 'Configured forecast_days ({req}) exceeds Open-Meteo availability — the last {gap} columns will show empty sunshine bars.',
    'openmeteo_history': 'Open-Meteo past data',
    'openmeteo_history_hint': 'Fills the past half of the chart from Open-Meteo when no station sensors are configured. Sends your Home Assistant location to Open-Meteo.',
    'openmeteo_history_unavailable': 'No past data: this card has no station sensors and Open-Meteo past data is off, so only the forecast can be shown. Turn on the option below, or add a station sensor, to enable the station and combination views.',
    'chart_style': 'Chart style',
    'chart_style_without_boxes': 'Without boxes (standard)',
    'chart_style_with_boxes': 'With boxes',
    'round_temp': 'Round temperatures',
    'disable_animation': 'Chart without transition',
    'disable_animation_hint': 'Disables the 500 ms transition when the chart redraws. Weather icons and scrolling are not affected.',
    'number_of_forecasts': 'Visible columns',
    'number_of_forecasts_hint': 'How many bars are visible at once (default 8 — fits both 7-day daily and an hour-window at hourly). 0 = fit all without scrolling. When loaded bars exceed visible, scroll horizontally for the rest.',
    'locale': 'Locale override',
    // v2.3 editor redesign (ADR-0023) — panel summaries, past-source
    // dropdown, clock dropdown, unit labels, helper texts.
    'past_source_label': 'Past data',
    'past_source_station': 'From my weather station (sensors)',
    'past_source_openmeteo': 'From Open-Meteo — I have no weather station',
    'chart_height': 'Chart height (px)',
    'number_of_forecasts_helper': 'Visible columns: 0 = show all without scrolling',
    'clock_label': 'Clock',
    'clock_off': 'Off',
    'clock_24h': '24-hour',
    'clock_24h_seconds': '24-hour + seconds',
    'clock_12h': '12-hour',
    'clock_12h_seconds': '12-hour + seconds',
    'main_elements_label': 'Elements',
    'unit_pressure_label': 'Show pressure in',
    'unit_speed_label': 'Show wind speed in',
    'unit_precipitation_label': 'Show precipitation in',
    'summary_connected': '{n} sensors connected',
    'summary_no_sensors': 'No sensors configured',
    'summary_openmeteo': 'Open-Meteo history (no station)',
    'summary_days': 'days',
    'summary_columns': 'columns',
    'summary_rows': 'rows',
    'summary_attributes': 'attributes',
    'summary_on': 'on',
    'summary_off': 'off',
  }
};

export default en;
