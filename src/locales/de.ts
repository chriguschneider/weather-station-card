// Per-language string table for de.
// One file per language so rollup can lazy-load on demand.
// Shape and fallback resolution rules live in ../locale.ts.
import type { LocaleEntry } from '../locale-types.js';

const de: LocaleEntry = {
  'tempHi': 'Temperatur',
  'tempLo': 'Nachttemperatur',
  'precip': 'Niederschlag',
  'sunshine': 'Sonne',
  'feelsLike': 'Gefühlt',
  'pressure_trend_rising_fast': 'Luftdruck schnell steigend',
  'pressure_trend_rising': 'Luftdruck steigend',
  'pressure_trend_stable': 'Luftdruck stabil',
  'pressure_trend_falling': 'Luftdruck fallend',
  'pressure_trend_falling_fast': 'Luftdruck schnell fallend',
  'pressure_influence_rising_fast': 'rasche Wetterbesserung',
  'pressure_influence_rising': 'Besserung wahrscheinlich',
  'pressure_influence_stable': 'Wetter bleibt stabil',
  'pressure_influence_falling': 'Verschlechterung wahrscheinlich',
  'pressure_influence_falling_fast': 'Sturmgefahr',
  'pressure_tooltip_template': '{trend} · Δ3h {delta} hPa · {influence}',
  'dew_point_band_raureif': 'Raureifgefahr',
  'dew_point_band_nebel': 'Nebelgefahr',
  'dew_point_band_schwuel': 'Schwül',
  'dew_point_band_tau': 'Taubildung möglich',
  'dew_point_band_komfort': 'komfortabel',
  'dew_point_tooltip_template': 'Taupunkt {td} {unit} · Abstand {spread} {unit} · {band}',
  'sun_strength_band_low': 'Niedrig',
  'sun_strength_band_moderate': 'Mässig',
  'sun_strength_band_high': 'Hoch',
  'sun_strength_band_very_high': 'Sehr hoch',
  'sun_strength_band_extreme': 'Extrem',
  'sun_strength_clear': 'klarer Himmel',
  'sun_strength_partly': 'teils bewölkt',
  'sun_strength_cloudy': 'bewölkt',
  'sun_strength_protection_advised': 'Sonnenschutz empfohlen',
  'sun_strength_night_reason': 'Sonne unter Horizont — UV ausgeblendet',
  'units': {
    'km/h': 'km/h',
    'm/s': 'm/s',
    'mph': 'mph',
    'Bft': 'Bft',
    'hPa': 'hPa',
    'mmHg': 'mm Hg',
    'mm': 'mm',
    'in': 'in'
  },
  'cardinalDirections': [
    'N', 'N-NO', 'NO', 'O-NO', 'O', 'O-SO', 'SO', 'S-SO',
    'S', 'S-SW', 'SW', 'W-SW', 'W', 'W-NW', 'NW', 'N-NW', 'N'
  ],
  'clear-night': 'Sternenklar',
  'cloudy': 'Bewölkt',
  'fog': 'Nebel',
  'hail': 'Hagel',
  'lightning': 'Gewitter',
  'lightning-rainy': 'Gewitter, Regen',
  'partlycloudy': 'Teils bewölkt',
  'pouring': 'Starkregen',
  'rainy': 'Regen',
  'snowy': 'Schnee',
  'snowy-rainy': 'Schneeregen',
  'sunny': 'Sonne',
  'windy': 'Windig',
  'windy-variant': 'Windig',
  'editor': {
    'temperature': 'Temperatur',
    'humidity': 'Luftfeuchtigkeit',
    'illuminance': 'Helligkeit',
    'precipitation': 'Niederschlag',
    'precipitation_rate': 'Niederschlagsrate',
    'pressure': 'Luftdruck',
    'wind_speed': 'Windgeschwindigkeit',
    'gust_speed': 'Böen',
    'wind_direction': 'Windrichtung',
    'uv_index': 'UV-Index',
    'dew_point': 'Taupunkt',
    'sunshine_duration': 'Sonnenscheindauer',
    'zero_degree_level': 'Nullgradgrenze',
    'required_marker': 'erforderlich',
    'title': 'Titel',
    'days': 'Tage (Station)',
    'forecast_days': 'Tage (Vorhersage)',
    'weather_entity': 'Wetter-Entität',
    'setup_heading': 'Einrichtung',
    'sensors_heading': 'Sensoren',
    'layout_heading': 'Aufbau',
    'style_heading': 'Stil & Farben',
    'units_heading': 'Einheiten',
    'advanced_heading': 'Erweitert',
    // Section headings — top-level
    'card_setup_heading': 'Karte einrichten',
    'station_sensors_heading': 'Sensoren deiner Wetterstation',
    'weather_forecast_heading': 'Wettervorhersage',
    'chart_section_heading': 'Diagramm',
    'live_panel_heading': 'Live-Anzeige',
    'actions_section_heading': 'Aktionen',
    // Per-section reset-to-defaults button
    'reset_section': 'Diese Sektion auf Standardwerte zurücksetzen',
    // Sub-section headings
    'chart_time_range_heading': 'Zeitraum & Auflösung',
    // Keys retained for backwards compatibility (third-party
    // translations that target the previous schema keep working).
    'mode_question_heading': 'Was zeigt die Karte?',
    'display_question_heading': 'Was wird angezeigt?',
    'appearance_heading': 'Aussehen',
    'tap_behavior_heading': 'Was passiert beim Tippen?',
    'card_heading': 'Karte',
    'display_heading': 'Anzeige',
    'chart_heading': 'Diagramm',
    'mode_label': 'Modus',
    'mode_station': 'Nur Station',
    'mode_forecast': 'Nur Vorhersage',
    'mode_combination': 'Station und Vorhersage kombiniert',
    'chart_type_label': 'Diagramm-Typ',
    'forecast_type_daily': 'Täglich',
    'forecast_type_today': 'Heute (24h)',
    'forecast_type_hourly': 'Stündlich',
    'open_documentation': 'Dokumentation öffnen',
    'main_panel_heading': 'Hauptpanel',
    'attributes_heading': 'Attributzeile',
    'chart_rows_heading': 'Diagrammzeilen',
    'tap_action_label': 'Klick-Aktion',
    'hold_action_label': 'Halten-Aktion',
    'double_tap_action_label': 'Doppelklick-Aktion',
    'chart_appearance_heading': 'Darstellung',
    'show_main': '"Jetzt"-Hauptpanel anzeigen',
    'show_temperature': 'Aktuelle Temperatur',
    'show_current_condition': 'Aktuelle Wetterlage',
    'show_attributes': 'Attributzeile anzeigen',
    'show_humidity': 'Luftfeuchtigkeit',
    'show_pressure': 'Luftdruck',
    'show_dew_point': 'Taupunkt',
    'show_wind_direction': 'Windrichtung',
    'show_wind_speed': 'Windgeschwindigkeit',
    'show_wind_gust_speed': 'Böen',
    'show_uv_index': 'UV-Index',
    'show_illuminance': 'Helligkeit',
    'show_precipitation': 'Niederschlag',
    'show_sun': 'Sonne (Auf-/Untergang)',
    'show_moon': 'Mond (Beleuchtung / Auf-/Untergang)',
    'show_zero_degree_level': 'Nullgradgrenze',
    'layout_board_label': 'Anordnung',
    'layout_board_hint': 'Zeilen zwischen den Spalten ziehen, um sie anzuordnen; Pfeiltasten gehen auch. Gespeichert als attributes_layout.',
    'layout_board_new_column': 'Neue Spalte',
    'layout_board_reset': 'Zurück zur automatischen Anordnung',
    'show_time': 'Uhr anzeigen',
    'show_time_seconds': 'Sekunden',
    'show_day': 'Wochentag',
    'show_date': 'Datum',
    'use_12hour_format': '12-Stunden-Format',
    'show_chart_icons': 'Bedingungs-Icon-Zeile',
    'show_chart_wind_direction': 'Windrichtung',
    'show_chart_wind_speed': 'Windgeschwindigkeit',
    'show_chart_date': 'Datum in der x-Achse',
    'show_chart_sunshine': 'Sonnenschein-Balken',
    'show_chart_sunshine_hint': 'Holt Sonnenscheindauer direkt von Open-Meteo (Standort aus Home Assistant). Keine zusätzlichen Sensoren nötig. In der Tagesansicht mit "Xh"-Beschriftung, im Stundenmodus nur als Balken (volle Höhe = volle Stunde Sonne).',
    'show_chart_mode_toggle': 'Ansicht-Umschalt-Button',
    'sunshine_availability': 'Sonnenstunden-Daten verfügbar: {past} Tage Vergangenheit, {future} Tage Vorhersage (Open-Meteo).',
    'sunshine_availability_pending': 'Sonnenstunden: Daten werden geladen — speichere und öffne den Editor erneut.',
    'sunshine_availability_warning': 'Konfigurierte Vorhersage-Tage ({req}) übersteigen Open-Meteo-Verfügbarkeit — die letzten {gap} Spalten zeigen leere Sonnenstunden-Balken.',
    'openmeteo_history': 'Open-Meteo-Verlaufsdaten',
    'openmeteo_history_hint': 'Füllt die Vergangenheits-Hälfte des Diagramms aus Open-Meteo, wenn keine Stationssensoren konfiguriert sind. Sendet den Home-Assistant-Standort an Open-Meteo.',
    'openmeteo_history_unavailable': 'Keine Vergangenheitsdaten: Diese Karte hat keine Stationssensoren und die Open-Meteo-Verlaufsdaten sind aus — daher kann nur die Vorhersage gezeigt werden. Aktiviere die Option unten oder füge einen Stationssensor hinzu, um die Stations- und Kombinationsansicht zu aktivieren.',
    'chart_style': 'Diagramm-Stil',
    'chart_style_without_boxes': 'Ohne Boxen (Standard)',
    'chart_style_with_boxes': 'Mit Boxen',
    'round_temp': 'Temperaturen runden',
    'disable_animation': 'Diagramm ohne Übergang',
    'disable_animation_hint': 'Schaltet die 500 ms-Übergangs-Animation beim Neuzeichnen aus. Wetter-Icons und Scrollen sind nicht betroffen.',
    'number_of_forecasts': 'Sichtbare Spalten',
    'number_of_forecasts_hint': 'Wie viele Balken auf einmal sichtbar sind (Default 8 — passt sowohl für 7 Tage täglich als auch für eine Stunden-Auswahl bei stündlich). 0 = alles zeigen ohne Scrollen. Wenn weniger Spalten sichtbar als Daten geladen sind, kann man horizontal scrollen.',
    'locale': 'Sprache (Override)',
    // v2.3 Editor-Redesign (ADR-0023) — Panel-Zusammenfassungen,
    // Quellen-Dropdown, Uhr-Dropdown, Einheiten-Labels, Helper-Texte.
    'past_source_label': 'Vergangenheits-Daten',
    'past_source_station': 'Von meiner Wetterstation (Sensoren)',
    'past_source_openmeteo': 'Von Open-Meteo — ich habe keine Wetterstation',
    'chart_height': 'Diagramm-Höhe (px)',
    'number_of_forecasts_helper': 'Sichtbare Spalten: 0 = alles ohne Scrollen zeigen',
    'clock_label': 'Uhr',
    'clock_off': 'Aus',
    'clock_24h': '24-Stunden',
    'clock_24h_seconds': '24 Stunden + Sekunden',
    'clock_12h': '12-Stunden',
    'clock_12h_seconds': '12 Stunden + Sekunden',
    'main_elements_label': 'Elemente',
    'unit_pressure_label': 'Luftdruck anzeigen in',
    'unit_speed_label': 'Wind anzeigen in',
    'unit_precipitation_label': 'Niederschlag anzeigen in',
    'summary_connected': '{n} Sensoren verbunden',
    'summary_no_sensors': 'Keine Sensoren konfiguriert',
    'summary_openmeteo': 'Open-Meteo-Verlauf (keine Station)',
    'summary_days': 'Tage',
    'summary_columns': 'Spalten',
    'summary_rows': 'Zeilen',
    'summary_attributes': 'Attribute',
    'summary_on': 'an',
    'summary_off': 'aus',
  }
};

export default de;
